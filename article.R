...
str(metadata(filtered)$sc3) # k = 12 estimated for article -> filtered
...
sc3_plot_silhouette(filtered, k = 3)

# Cluster stability:
sc3_plot_cluster_stability(filtered, k = 12)

# Cluster consensus:
sc3_plot_consensus(filtered, k = 3)
...
setwd("/home/rpg18/Desktop/SRP_article/PIPELINE/Pipeline_SecondSteps/Sc3/Results_attempts_trial/Authors/NEW/new.R/Norm_postfilter/")
sc3_plot_markers(
  filtered, k = 8, 
  show_pdata = c(
    "cell_type", 
...
plotDiffusionMap(example_sce, colour_by = "cell_type", size_by = "SOX11 ")
...
for(k in row.names(lista)){
 ...
 # else if (meh %in% 10){cluster10<-append(cluster10,k)}
  #else if (meh %in% 11){cluster11<-append(cluster11,k)}
 # else if (meh %in% 12){cluster12<-append(cluster12,k)}
 # else if (meh %in% 13){cluster13<-append(cluster13,k)}
...
write.table(cluster10, file = "cells_cluster10.txt", col.names = NA)
write.table(cluster11, file = "cells_cluster11.txt", col.names = NA)
write.table(cluster12, file = "cells_cluster12.txt", col.names = NA)
write.table(cluster13, file = "cells_cluster13.txt", col.names = NA)
